...
void HG25Q256_Write_Multiple_Pages(uint32_t start_address, const uint8_t* data, uint32_t total_length) {
    ...
    uint32_t current_addr = start_address; // 当前写入地址
    ...
    while (bytes_written < total_length) {
        ...
        if ((current_addr & (SECTOR_SIZE - 1)) == 0) {
            HG25Q256_Erase_Sector(current_addr);
						
        }
        
        // 写入当前页数据
        HG25Q256_Write_Page(&data[bytes_written], current_addr, length_to_write);
...
void ReadAndDisplayDataFromHG25Q256(uint32_t start_address) {
    uint32_t flash_addr = start_address;
    uint32_t remain_len = 51200;	// 160×160×2=51200字节（与TFT匹配）
    uint8_t  animation_buf[1024];
		uint8_t rbuf[51200];
    uint32_t bytes_read = 0;              // 已读取字节数
	
		memset(rbuf, 0, sizeof(rbuf));
		memset(animation_buf, 0, sizeof(animation_buf));
		// 循环读取Flash数据到rbuf
    while (remain_len > 0) {
        uint32_t read_len = (remain_len > 1024) ? 1024 : remain_len;

        // 启动DMA读
        dma_transfer_complete = 0;
        int ret = HG25Q256_Read_Data_DMA(animation_buf, read_len, flash_addr);
        if (ret != 0) {
        ...
        if (timeout == 0) {
            ...
            break;
				...
				memcpy(&rbuf[bytes_read], animation_buf, read_len);
		...
		// 显示完整图片（传rbuf而非animation_buf，等待DMA完成）
    if (bytes_read == 51200) { // 确保读取到完整数据
        //Picture_Display1(rbuf, 51200);
    } else {
        printf("读取数据不完整：%d/%d字节\n", bytes_read, TOTAL_BYTES);
    }	
...
// 定义数据读取和显示函数
void Animation_Display(uint32_t start_address) {
    uint32_t flash_addr = IMAGE1_FLASH_ADDR;
    uint32_t remain_len = 51200;
    uint8_t  animation_buf[1024];
    uint32_t frame = 0;
    ...
    while (remain_len > 0) {
        ...
        if (timeout == 0) {
            ...
            break;
				 ...
				 // 3. 打印本帧数据
        /*printf("帧%d, 地址0x%06lX, 长度%d, 数据:\n", frame + 1, flash_addr, read_len);
        for (uint32_t i = 0; i < read_len; i++) {
            printf("%02X ", animation_buf[i]);
            if ((i + 1) % 16 == 0) printf("\n");
        }
        printf("\n");*/
				
        // 3. 显示动画帧（假设一帧数据就是 read_len 字节）
        //Picture_Display1(animation_buf, read_len);
        HAL_Delay(50); // 可根据实际需要延时
    ...
    //printf("动画显示完成, 总帧数: %lu\n", frame);
}
void ReadAndDisplayData(uint32_t start_address) {

    uint8_t rbuf[3206];
		uint8_t  animation_buf[1024];
    uint32_t addr = IMAGE1_FLASH_ADDR;
		uint32_t remain_len = 3206;
     ...
     while (remain_len > 0) {
        ...
        // 1. 启动DMA读
        dma_transfer_complete = 0;
        int ret = HG25Q256_Read_Data_DMA(animation_buf, read_len, addr);
        if (ret != 0) {
            printf("DMA读启动失败, ret=%d\n", ret);
            // 可选: break 或 continue
            break;
        ...
        if (timeout == 0) {
            ...
            break;
				...
				memcpy(rbuf, animation_buf, read_len);
