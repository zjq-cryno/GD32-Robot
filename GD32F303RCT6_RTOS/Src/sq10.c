...
uint8_t servo_status[6] = {0};    // 舵机状态缓存（索引=舵机ID，1-6）
    ...
    switch (cmd) {
        ...
        case 0x2E: // 参数保存应答
            ...
            if (length == 6) {
								...
								servo_status[idx] = frame[4];  // 保存状态值
								// 检测堵转标志（bit0=1）
								if (frame[4] & 0x01) {
										Servo_ReleaseTorque(id);  // 堵转→卸力
										Servo_QueryStatus(id);    // 二次查询清除标志
								}
            }
            break;
				case 0x51: // 当前位置查询应答
...
void servo_angle_speed(UART_HandleTypeDef *huart, uint8_t servo_id, int16_t angle, int16_t speed, uint8_t array[ARRAY_SIZE]) {
    ...
    // 发送命令
		if (osMutexWait(uart5MutexHandle, osWaitForever) == osOK) {
...
void init_all_servos(UART_HandleTypeDef *huart) {
    ...
    #define ENABLE_POWER_ON_ZERO_CALIB 1 // 1=启用，0=禁用
...
uint8_t Servo_ConfigStallDetection(uint8_t servo_id) {
    ...
    // 指令帧：AB ID 07 30 05 00 cks（数据0x0500=500ms）
    uint8_t tx_frame[7] = {0xAB, servo_id, 0x07, 0x30, 0x05, 0x00, 0x00};
    tx_frame[6] = calculate_checksum(tx_frame, 7);  // 计算校验位
	
    HAL_UART_Transmit(&huart5, tx_frame, 7, HAL_MAX_DELAY);
    printf("Servo%d: 配置堵转检测时间500ms\r\n", servo_id);
    return 0;
}
...
uint8_t Servo_QueryStatus(uint8_t servo_id) {
    if (servo_id < 1 || servo_id > 6) {
        uart1_print("Servo%d: 无效ID", servo_id);
        return 1;
    }
    // 指令帧：AB ID 05 50 cks（读数据指令帧）
    uint8_t tx_frame[5] = {0xAB, servo_id, 0x05, 0x50, 0x00};
    tx_frame[4] = calculate_checksum(tx_frame, 5);  // 计算校验位
    
    My_UART_Transmit(&huart5, tx_frame, 5);
    return 0;
}
/**
 * @brief 舵机卸力（无阻尼关扭矩，不锁舵）
 * @param servo_id：舵机ID（1-250）
 * @return 0=成功，1=失败
 */
uint8_t Servo_ReleaseTorque(uint8_t servo_id) {
    ...
    // 指令帧：AB ID 06 40 00 cks（扭矩控制指令）
    uint8_t tx_frame[6] = {0xAB, servo_id, 0x06, 0x40, 0x00, 0x00};
    tx_frame[5] = calculate_checksum(tx_frame, 6);  // 计算校验位
    
    My_UART_Transmit(&huart5, tx_frame, 6);
    uart1_print("Servo%d: 检测到堵转，已卸力", servo_id);
    return 0;
}
